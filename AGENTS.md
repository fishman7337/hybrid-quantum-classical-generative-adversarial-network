...
- **Project:** Quantum-Enhanced GANs (HQCGAN Research)
...
- Install full experiment dependencies with `python -m pip install -e ".[experiment,quantum,dev]"` only for ML or quantum experiment work.
- Set `QGAN_FULL_STUDY=1` only when the long optimisation is explicitly intended and suitable compute is available.

## Managed artifacts and data

- Keep local data, checkpoints, models, generated figures, and machine-specific reports out of Git; retain only documented placeholders and approved research artifacts.
- Validate `notebooks/Quantum-GAN.ipynb` with `python scripts/validate_notebooks.py`; do not manufacture execution outputs.

## Integration and claim boundaries

- The default workflow is a bounded CPU-safe study; a full GPU experiment was not validated in the portfolio audit.
- This project is distinct from the quantum-computing sustainability report and does not support that report's power-consumption claims.
- Secrets and machine-specific paths belong in `.env`, never in source files.
- **Verified local capability:** Bounded CPU-safe experiment path and notebook validation.
- **Known boundary:** Full GPU study was not run; distinct from quantum-sustainability work.
